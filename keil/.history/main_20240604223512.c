...
void main()
{
	...
	uchar TH = Read_EEPROM_DS18B20() >> 8;
	uchar TL = Read_EEPROM_DS18B20();
...
/**
 * @description: 显示温度
 * @return {char} tempreatuer 用于比较温度上下限的只有个位和十位的带符号温度数据
 */
char displaytemp()
{
	char tempreatuer = 0;				// 用于比较温度上下限的只有个位和十位的带符号温度数据
	...
	int original = Read_temp_DS18B20(); // 原始温度数据，16位补码，高5位符号位，低11数据位
	if (original < 0)					// 负
	{
...
/**
 * @description: 设置阈值
 * @param {char} TH_or_TL 阈值
 * @return {char} TH_or_TL 阈值
 */
char set_limit(char TH_or_TL)
{
	uchar unit = 0;		  // 个位
